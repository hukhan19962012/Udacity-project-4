...
export async function createTodo(event: APIGatewayProxyEvent, newTodoData: TodoCreateModel): Promise<TodoItem> {
    const todoId = uuid.v4();
    const userId = getUserId(event);
    const createdDate = new Date().toISOString();
    const done = false;
    const newTodo: TodoItem = { todoId, userId, createdAt: createdDate, done, ...newTodoData };
